...
test("LIVE 20. Director puede decir sí solo con umbrales y viabilidad satisfechos", () => { const result = analysis(); assert.equal(result.director.analysis_decision.status, "yes"); assert.ok(result.director.sports_verdict.sports_score >= 68); assert.ok(result.director.analysis_confidence_score >= 58); assert.equal(result.director.analysis_decision.operationally_actionable, true); assert.equal(result.director.market_viability.status, "pending"); });
...
test("LIVE 22b. una línea live distinta solo se activa tras evaluación independiente", () => { const payload = oddsPayload(); payload[0].bookmakers[0].bets[0].values[0].value = "Over 13.5"; const result = analyzeLiveMatch({ analysisId: "line", competitionKey: "colombiaPrimeraA", fixture: fixture(), statistics: statistics(), liveOddsPayload: payload, fixtureFetchedAt: FETCHED, statisticsFetchedAt: FETCHED, oddsFetchedAt: FETCHED, analyzedAt: NOW }); const corners = result.market_assessments.find((item) => item.market_family === "corners"); assert.equal(corners.candidate.line, 12.5); assert.equal(corners.operational_candidate.line, 13.5); assert.equal(corners.operational_candidate.candidate_origin, "exact_live_line_reanalysis"); assert.notEqual(corners.operational_candidate.candidate_id, corners.candidate.candidate_id); });
test("LIVE 22c. análisis deportivo funciona sin cuota y expone la línea no disponible", () => { const result = analyzeLiveMatch({ analysisId: "no-price", competitionKey: "colombiaPrimeraA", fixture: fixture(), statistics: statistics(), fixtureFetchedAt: FETCHED, statisticsFetchedAt: FETCHED, analyzedAt: NOW }); assert.equal(result.status, "success"); assert.equal(result.director.price_assessment.status, "line_not_live"); assert.equal(result.director.analysis_decision.status, "wait"); });
...
test("LIVE 33. UI visible ofrece actualización, partido manual y guardado en español", async () => { const source = await readFile(path.resolve(testingDirectory, "../../app/atlas-live.js"), "utf8"); assert.match(source, /Actualizar EN VIVO/); assert.match(source, /partido EN VIVO conocido/); assert.match(source, /Guardar pronóstico EN VIVO/); });
test("LIVE 34. navegación separa Atlas EN VIVO del modo prepartido", async () => { const source = await readFile(path.resolve(testingDirectory, "../../app/atlas-functional-client.js"), "utf8"); assert.match(source, /mainMode === "live"/); assert.match(source, />Atlas EN VIVO</); });
