...
test("LIVE 35. API LIVE exige acceso local y expone GET y POST", async () => { const source = await readFile(path.resolve(testingDirectory, "../../app/api/football/live/route.js"), "utf8"); assert.match(source, /isLocalRequest/); assert.match(source, /export async function GET/); assert.match(source, /export async function POST/); });
