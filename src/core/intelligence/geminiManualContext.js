...
export function buildGeminiResearchPrompt({ fixture, competition, market, selection = null, competitiveContext = null, oddsQuote, verifiedData = [], missingData = [], risks = [], analyzedAt = new Date().toISOString() }) {
  ...
  return [
    ...
    "Si dispones de una función de búsqueda o navegación web en esta sesión, tienes la OBLIGACIÓN de usarla para investigar este fixture antes de responder; no es opcional.",
    "Prioriza fuentes oficiales (clubes, ligas, competición) y periodísticas reconocidas y fiables sobre blogs, foros o agregadores genéricos.",
    "Solo si genuinamente no dispones de ninguna función de búsqueda o navegación en esta sesión, decláralo explícitamente al inicio de tu respuesta y limita tu respuesta a lo que puedas verificar de otro modo, marcando el resto como NO VERIFICADO.",
    "La ausencia de confirmación para un dato puntual no debe impedir que investigues y reportes el resto de los datos solicitados: evalúa cada dato de forma independiente.",
    "Prioriza información publicada en las últimas 24 horas cuando exista, sin descartar hechos confirmados más antiguos que sigan vigentes.",
    "Si no puedes verificar un dato concreto con información actual, decláralo explícitamente como NO VERIFICADO únicamente para ese dato, en vez de omitirlo en silencio o dejar de investigar los demás.",
    "",
    "REGLA CRÍTICA DE INTEGRIDAD — OBLIGATORIA",
    "Actúa como auditor estricto de datos.",
    "Está prohibido inventar, asumir, deducir, completar por inferencia o reutilizar información antigua para rellenar vacíos.",
    "Un dato solo puede aparecer en HECHOS CONFIRMADOS si existe evidencia actual, explícita, específica y verificable que respalde ese mismo dato para el fixture, competición, temporada y fecha investigados.",
    "Presta especial atención a: árbitro designado; lesiones; suspensiones; convocatorias; alineaciones; estadísticas actuales; cambios de entrenador; sede y condiciones del partido.",
    "No reutilices datos de partidos anteriores o temporadas anteriores como si pertenecieran al fixture actual.",
    "No conviertas una inferencia en hecho confirmado.",
    "Si el dato es plausible pero no confirmado y existe fuente identificable: INFORMACIÓN PROBABLE.",
    "Si existen fuentes contradictorias: CONTRADICCIONES.",
    "Si no existe evidencia actual, específica y verificable: DATOS NO ENCONTRADOS.",
    "Nunca rellenes un vacío para hacer el análisis más completo. Es preferible devolver menos información que introducir información falsa.",
    "",
    "REGLA ESPECIAL DE ÁRBITRO",
    "Una designación arbitral solo puede aparecer en HECHOS CONFIRMADOS cuando una fuente actual vincule explícitamente el nombre del árbitro con el FIXTURE EXACTO investigado.",
    "No infieras árbitro usando partidos anteriores, perfiles estadísticos, historial del árbitro, coincidencias de equipos o resultados de búsqueda relacionados.",
    "",
    "FUENTES",
    "Cada HECHO CONFIRMADO debe conservar HECHO, IMPACTO, FUENTE, URL y FECHA.",
    "La URL debe respaldar específicamente el hecho. Una portada, página genérica o resultado de búsqueda no es suficiente.",
    "",
    "FORMATO DE SALIDA OBLIGATORIO:",
    "Antes de las cinco secciones, incluye una única línea con este formato:",
    "ESTADO DE BÚSQUEDA WEB: [USADA o NO DISPONIBLE]",
    "Sustituye el contenido entre corchetes por UN SOLO estado.",
    "Escribe USADA si efectivamente utilizaste búsqueda o navegación web.",
    "Escribe NO DISPONIBLE únicamente si genuinamente no dispones de esa capacidad.",
    "Nunca escribas ambos estados.",
    "Mantén exactamente estas cinco secciones y este orden:",
    "",
    "HECHOS CONFIRMADOS",
    ...
    "HECHO: [un único hecho probable, identificado expresamente como probable]",
    "IMPACTO: [fortalece / debilita / invalida / sin cambio / mixto] + explicación breve",
    "FUENTE: [nombre concreto de la fuente]",
    "URL: [URL directa completa que respalda específicamente el hecho]",
    "FECHA: [fecha de publicación o actualización de esa fuente]",
    ...
    "Lista únicamente los datos materiales que realmente no pudiste verificar.",
    "",
    "REGLAS ESTRUCTURALES OBLIGATORIAS:",
    "- HECHO + IMPACTO + FUENTE + URL + FECHA forman UN MISMO BLOQUE.",
    "- Cada HECHO nuevo inicia un bloque nuevo.",
    "- NO separes FUENTE, URL o FECHA como elementos independientes.",
    "- No uses Google Sports Data, Google, resultados de búsqueda u otra referencia genérica como fuente si no existe una URL directa concreta.",
    "- Si no puedes proporcionar una URL directa verificable, NO presentes el dato como confirmado: pásalo a DATOS NO ENCONTRADOS.",
    "- Una portada general no es suficiente; la URL debe respaldar concretamente el hecho.",
    "- No inventes URLs ni reutilices una URL que no respalde el hecho.",
    "- La FECHA debe pertenecer a la fuente citada en el mismo bloque.",
    "- INFORMACIÓN PROBABLE puede incluir fuente y URL, pero debe permanecer identificada como probable.",
    "- No recomiendes apostar ni generes probabilidades.",
    "- No cambies el fixture, los equipos, el mercado, la línea ni la cuota proporcionados.",
  ].join("\n");
}

// Segundo prompt del flujo manual (Gemini Pro normal): no investiga, solo
// depura y da formato a lo que ya entregó Gemini Pro + Deep Research. Nunca
// llama a Gemini por API; el usuario copia/pega este texto manualmente.
export function buildGeminiCleanupPrompt({ fixture, competition, market, selection = null, analyzedAt = new Date().toISOString() }) {
  if (!fixture?.fixtureId) throw new TypeError("El prompt de depuración requiere un fixture verificado.");
  const home = fixture.teams?.home?.name || "Equipo local no disponible";
  const away = fixture.teams?.away?.name || "Equipo visitante no disponible";
  return [
    "DEPURACIÓN Y FORMATEO PARA ATLAS (Gemini Pro normal)",
    "Esta tarea NO es una nueva investigación: trabaja únicamente sobre el texto que se pega más abajo, producido por Gemini Pro + Deep Research.",
    `Fixture ID: ${fixture.fixtureId}`,
    `Competición: ${competition?.localName || fixture.competition?.name || "No disponible"}`,
    `Partido: ${home} vs ${away}`,
    `Mercado: ${market?.market_label || market?.market_family || "Abierto"}`,
    `Selección exacta que Atlas está estudiando: ${selection?.selection || "No disponible"}`,
    `Línea exacta: ${selection?.line || market?.line || "No disponible"}`,
    `Hora exacta de esta depuración: ${analyzedAt}`,
    "",
    "REGLAS DE DEPURACIÓN — OBLIGATORIAS:",
    "- No vuelvas a investigar desde cero: usa exclusivamente el texto pegado más abajo.",
    "- No recomiendes apuestas.",
    "- No generes probabilidades nuevas.",
    "- No modifiques la probabilidad deportiva de Atlas (estimated_probability).",
    "- No modifiques sports_score.",
    "- No añadas conclusiones nuevas que el texto pegado no contenga.",
    "- Depura contradicciones internas del texto pegado.",
    "- Conserva solo información útil para este fixture exacto.",
    "- Separa hechos confirmados de información probable.",
    "- No presentes como confirmado algo que la fuente citada no respalde.",
    "- Conserva FUENTE, URL y FECHA de cada hecho tal como aparecen en el texto pegado.",
    "- Si dos fuentes fiables discrepan entre sí sobre el mismo dato, mueve ese dato a CONTRADICCIONES.",
    "- Si un dato no pudo verificarse en el texto pegado, muévelo a DATOS NO ENCONTRADOS.",
    "- No inventes información que no esté en el texto pegado.",
    "- No inventes cuotas.",
    "",
    "PEGA AQUÍ LA RESPUESTA COMPLETA DE GEMINI PRO + DEEP RESEARCH (sin resumir ni recortar):",
    "[PEGAR AQUÍ LA RESPUESTA COMPLETA DE DEEP RESEARCH]",
    "",
    "FORMATO DE SALIDA OBLIGATORIO — exige únicamente estas secciones, en este orden:",
    "Conserva la línea ESTADO DE BÚSQUEDA WEB exactamente como aparezca en el texto pegado (USADA o NO DISPONIBLE); no la alteres ni la inventes si falta.",
    "",
    "ESTADO DE BÚSQUEDA WEB: [conserva exactamente USADA o NO DISPONIBLE según el informe original]",
    "",
    "HECHOS CONFIRMADOS",
    "HECHO: [un único hecho verificable]",
    "IMPACTO: [fortalece / debilita / invalida / sin cambio / mixto] + explicación breve",
    ...
    "",
    "DATOS NO ENCONTRADOS",
    "Lista únicamente los datos materiales que realmente no pudiste verificar en el texto pegado.",
    "",
    "PROHIBIDO EN TU RESPUESTA:",
    "- Introducción.",
    "- Conclusión.",
    "- Recomendación de apuesta.",
    "- Porcentajes o probabilidades nuevas.",
    "- Cualquier alteración de estimated_probability.",
    "- Cualquier alteración de sports_score.",
    "",
    "No respondas nada fuera de las cinco secciones anteriores y la línea ESTADO DE BÚSQUEDA WEB.",
  ].join("\n");
}
